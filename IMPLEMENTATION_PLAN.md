...
- [x] 求解缓存命中/未命中日志；超时降级（返回单趟启发式结果而非 504）。
- [x] 错误处理（两端顶层 ErrorBoundary）、加载/空/错误态、权限边界（401 清 token）。
- [x] 性能：`<Instances>` 实例化 + 商品列表虚拟滚动（admin AntD virtual / 装箱端 react-virtual）+ bundle 代码分割。
- [x] 物品旋转交互展示（点选高亮 + 朝向信息卡）；求解异步化（WebSocket 推送，失败退回 HTTP）。
